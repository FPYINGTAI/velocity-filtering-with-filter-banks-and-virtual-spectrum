...
%     for flag=1:100
        ...
        SNR=6;     
        I=10^(SNR/20);%目标幅度
        total_frame_cv=5;%cv模型总帧数
%         total_frame_cv=total_frame/2;
        total_frame=total_frame_cv*2;%观测总帧数/2;
        x_max=80;
        y_max=80;%观测总范围
...
%         
%     end
%     PD(total_frame)=PDflag/flag;
%     RMSE(total_frame)=sqrt(SIGMArmse/2/PDflag);
% 
ex=0;
ex2=0;
for i=30:74
...
% for i=33:25
%     for j=4:14
%         ex=ex-output_matrix(i,j,w_filter,v_filter,moving_point_filter);
%         ex2=ex2-output_matrix(i,j,w_filter,v_filter,moving_point_filter)^2;
%     end
% end
%%        
        %画出积累后的图
